...
| `skipPermissions` | boolean | `true` | Enables `--skip-permissions-unsafe` for autonomous runs |
...
Factory Droid normally asks for permission before executing tool actions. By default, Ralph TUI enables `skipPermissions = true` for autonomous operation. To require manual approvals, disable it:
...
With skip permissions enabled (the default), the agent runs without interactive approvals. Set `skipPermissions = false` if you need manual confirmation of tool actions.
</Callout>
